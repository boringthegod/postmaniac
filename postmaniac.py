        ...
        if item['document']['documentType'] == 'request':
            if 'publisherHandle' in item['document'] and item['document']['publisherHandle']:
                if 'slug' in item['document']['workspaces'][0]:
                    # Construit l'URL
                    urlworkspace = 'https://www.postman.com/' + item['document']['publisherHandle'] + '/workspace/' + \
    ...
    def find_croustillant(datacr):
        if isinstance(datacr, dict):
            for key, value in datacr.items():
                if key in ["voucher", "username", "password", "email", "token", "accesskey", "creditCard", "creditcard",
                           "phone", "address", "mobilephone", "cellPhone", "code", "authorization_code", "client_id",
                           "client_secret", "name", "apikey", "customer_email", "api_key", "api_secret", "apisecret",
        ...
        if datamode == "params" and len(requestresp['data']["data"]) > 0:
            ...
            for nom in datas:
                ...
                if nom['key'] in ["voucher", "username", "password", "email", "token", "accesskey", "creditCard",
                                  ...
                                  "customer_email",
                                  "api_key", "api_secret", "apisecret", "hash", "paypal_token", "identity", "phoneHome",
